...
class CytoDLPointPretrain(pl.LightningModule):
    def __init__(
        self,
        reconstructor,
    ...
    def configure_optimizers(self):
        ...
        cosine_scheduler = CosineAnnealingLR(
            optimizer,
            T_max=self.optimizer_cfg.epochs - warmup_epochs,
    ...
    def load_pretrained_weights(self, checkpoint_path):
        ...
        logger.info(f"Loaded {len(state_dict) - len(unexpected)} keys from checkpoint")

        if missing:
            logger.info(f"Missing {len(missing)} keys; using random initialization.")
        if unexpected:
            logger.info(f"Ignored {len(unexpected)} unexpected keys.")

    def training_step(self, batch, batch_idx):
        pts = self.transform(batch["points"])
    ...
    def on_train_epoch_end(self):
        ...
        logger.info(
            ...
            f"Loss: {loss_epoch:.4f} | Recon: {loss_recon:.4f}"
        )
